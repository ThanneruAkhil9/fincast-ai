...
section[data-testid="stSidebar"] .stMultiSelect label { color: #94a3b8 !important; font-size: 0.75rem; letter-spacing: 0.08em; text-transform: uppercase; }
...
.fincast-header h1 { margin:0; font-size: 2rem; font-weight: 700; letter-spacing: -0.02em; }
...
/* Section titles */
.section-title { font-size:1.1rem; font-weight:600; color:#0f172a; margin:1.5rem 0 0.75rem; letter-spacing:-0.01em; }
...
with st.sidebar:
    ...
    st.divider()
    st.caption("**Stack:** Python • DuckDB • Plotly • Streamlit")
...
if page == "🏠 Executive Dashboard":
    ...
    col4.metric("Anomalies Detected",   anomaly["summary"].get("total_anomalies", 0),
                f"{anomaly['summary'].get('critical_count',0)} critical",
                delta_color="inverse")
    ...
    dept_summary["color"]    = dept_summary["variance"].apply(
...
# ─────────────────────────────────────────────────────────────
    ...
    if fc_df.empty:
        st.warning("No forecast data available. Install statsmodels for Holt-Winters forecasting.")
        st.stop()
    ...
    fig = go.Figure()
    fig.add_scatter(
        x=act_monthly["period"], y=act_monthly["actual"],
        name="Actuals", line=dict(color=COLORS["primary"], width=2),
    )
    fig.add_scatter(
        x=fc_monthly["period"], y=fc_monthly["forecast"],
...
# ─────────────────────────────────────────────────────────────
    ...
    st.caption("Ask anything about your financials — powered by AI + DuckDB")
    ...
    # Handle pending (from suggestion button)
...
# ─────────────────────────────────────────────────────────────
    ...
    if timings:
        ...
        for i, (agent, t) in enumerate(timings.items()):
            [c1, c2, c3, c4][i].metric(f"{agent.title()} Agent", f"{t}s")
